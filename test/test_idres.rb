  ...
  class Consumer
    class IdResHandler
      ...
      class CheckForFieldsTest < Minitest::Test
        ...
        OPENID2_SIGNED =
          OPENID1_SIGNED + %w[response_nonce claimed_id assoc_handle
                              op_endpoint]
        ...
        1.times do # so as not to bleed into the outer namespace
          ...
          [[],
           ['foo'],
           %w[bar baz]].each do |signed_fields|
            test = lambda do
        ...
        1.times do
          [['openid1', OPENID1_NS, OPENID1_FIELDS],
           ...
           ['openid2', OPENID2_NS, OPENID2_FIELDS]].each_with_index do |(ver, ns, all_fields), i|
        ...
        1.times do
          [['openid1', OPENID1_NS, OPENID1_FIELDS, OPENID1_SIGNED],
           ...
           ['openid2', OPENID2_NS, OPENID2_FIELDS,
            OPENID2_SIGNED]].each_with_index do |(ver, ns, all_fields, signed_fields), i|
      ...
      class ReturnToArgsTest < Minitest::Test
        ...
        def check_return_to_args(query)
          ...
          class << idres
            def verify_return_to_base(unused); end
          end
      ...
      class ReturnToVerifyTest < Minitest::Test
        ...
        def test_good_return_to
          ...
          [[base, {}],
           ...
           ['http://example.janrain.com/./path', {}]].each do |return_to, args|
      ...
      class CheckAuthResponseTest < Minitest::Test
        ...
        def test_invalidate_no_store
          ...
          assert_log_matches("Received 'invalidate_handle'",
                             'Unexpectedly got "invalidate_handle"') do
            call_process
          end
        end
      end
      ...
      class NonceTest < Minitest::Test
        ...
        def test_openid1_success
          [{},
           ...
           { 'openid.ns' => OPENID11_NS }].each do |args|
          ...
          assert_log_matches('Error attempting to use stored',
            ...
            assert_raises(verified_error) do
              call_verify_modify({ 'ns' => OPENID1_NS,
                                   'identity' => @endpoint.local_id }) do |idres|
                idres.instance_def(:discover_and_verify) do |_claimed_id, _endpoints|
