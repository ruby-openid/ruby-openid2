...
module OpenID
  class AssociationTestCase < Minitest::Test
    ...
    def test_round_trip
      ...
      %i[handle secret lifetime assoc_type].each do |attr|
    ...
    def test_expires_in
      ...
      assert_equal(0, @assoc.expires_in(Time.now.to_i + 10_000), 'negative expires_in')
    end
    ...
    def test_from_expires_in
      ...
      %i[handle secret assoc_type].each do |attr|
    ...
    def test_sign_sha1
      ...
      [['HMAC-SHA256', "\xfd\xaa\xfe;\xac\xfc*\x988\xad\x05d6-\xeaVy\xd5\xa5Z.<\xa9\xed\x18\x82\\$\x95x\x1c&"],
       ['HMAC-SHA1', "\xe0\x1bv\x04\xf1G\xc0\xbb\x7f\x9a\x8b\xe9\xbc\xee}\\\xe5\xbb7*"]].each do |assoc_type, expected|
  ...
  class AssociationNegotiatorTestCase < Minitest::Test
    ...
    def test_copy
      neg = AssociationNegotiator.new([%w[HMAC-SHA1 DH-SHA1]])
      neg2 = neg.copy
    ...
    def test_add_allowed
      ...
      assert(neg.allowed?('HMAC-SHA256', 'DH-SHA256'))
      assert(!neg.allowed?('HMAC-SHA256', 'no-encryption'))
      assert_equal(neg.get_allowed_type, %w[HMAC-SHA1 DH-SHA1])
    end

    def test_bad_assoc_type
      assert_raises(ProtocolError) do
        AssociationNegotiator.new([%w[OMG Ponies]])
      end
    end

    def test_bad_session_type
      assert_raises(ProtocolError) do
        AssociationNegotiator.new([%w[HMAC-SHA1 OMG-Ponies]])
      end
    end
